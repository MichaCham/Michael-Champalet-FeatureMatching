namespace Michael.Champalet.FeatureMatching;
...
public class ObjectDetection
{
        ...
        objectImageData, IList<byte[]> imagesSceneData)
    {
        ...
        IList<ObjectDetectionResult> objectSetectionList = new List<ObjectDetectionResult>();
        var tasks = new List<Task<ObjectDetectionResult>>();

        foreach (var sceneData in imagesSceneData)
        {
            var task = Task.Run(() =>
            {
                var result = DetectObjectInScene(objectImageData, sceneData);
                return result;
            });
            tasks.Add(task);
        }
        ...
        foreach (var task in tasks)
        {
            ...
            objectSetectionList.Add(result);
        }

        return objectSetectionList;
    }
}